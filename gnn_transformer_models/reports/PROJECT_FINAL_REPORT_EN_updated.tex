...
\hline
Transformer (HEATransformer) & $-$0.238 & 63.43 & 41.68 \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Classical ML models perform best}: Gradient Boosting and optimized SVR achieve the highest $R^2$ values (0.67 and 0.59, respectively), demonstrating that well-tuned classical methods remain competitive even when compared to more complex deep-learning architectures.
  ...
  \item \textbf{Transformer model struggles}: The HEATransformer achieves a negative $R^2$ value ($-$0.238), indicating that the model performs worse than a simple mean predictor. This poor performance likely stems from the limited dataset size relative to the model complexity, insufficient training, or suboptimal hyperparameter settings.
\end{itemize}
